...
void menuSearching(Pesawat daftarPesawat[], int &jumlahPesawat) {
    ...
    while (!kembali) {
        ...
        cout << "  +------------------------------------------+\n";
        cout << "  Pilihan: ";
        cin >> pilihanCari; cin.ignore();

        if (pilihanCari == 1) {
            int idCari;
            cout << "  Masukkan ID yang dicari: "; cin >> idCari; cin.ignore();
            binarySearchID(daftarPesawat, jumlahPesawat, &idCari);
            cout << "  Tekan Enter untuk kembali..."; cin.get();

        } else if (pilihanCari == 2) {
            string kataCari;
            cout << "  Masukkan nama maskapai yang dicari: "; getline(cin, kataCari);
...
bool registerUser(Pengguna daftarUser[], int &jumlahUser, int MAX_USER) {
    ...
    if (regNama.empty() || regPass.empty()) { cout << "\n  [!] Nama dan Password tidak boleh kosong!\n"; return false; }
    for (int i = 0; i < jumlahUser; i++)
        if (daftarUser[i].nama == regNama) { cout << "\n  [!] Username sudah terdaftar!\n"; return false; }
...
Pengguna* loginUser(Pengguna daftarUser[], int &jumlahUser) {
    ...
    while (percobaan < 3) {
        ...
        for (int i = 0; i < jumlahUser; i++)
            if (daftarUser[i].nama == inputNama && daftarUser[i].password == inputPass && daftarUser[i].aktif)
                return &daftarUser[i];
        percobaan++;
...
void tambahPesawat(Pesawat daftarPesawat[], int &jumlahPesawat, int &idCounter, int MAX_PESAWAT) {
    ...
    Pesawat baru; baru.id = idCounter++;
    ...
    cout << "  Total kapasitas armada kini: " << totalKapasitas(daftarPesawat, 0, jumlahPesawat) << " penumpang\n";
}
...
void lihatPesawat(Pesawat daftarPesawat[], int jumlahPesawat) {
    ...
    tampilTabelPesawat(daftarPesawat, jumlahPesawat);
    cout << "  Total kapasitas seluruh armada: " << totalKapasitas(daftarPesawat, 0, jumlahPesawat) << " penumpang\n";
}
...
int main() {
    ...
    while (programBerjalan) {
        ...
        while (!sudahLogin) {
            ...
            cout << "  +-----------------------------+\n";
            cout << "  Pilihan: ";
            int pilihanAwal; cin >> pilihanAwal; cin.ignore();
